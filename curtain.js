...
y= Math.sqrt(cm), 
g= Math.round(cm*cm)/10,
h= Math.ceil(49.13), 
kp= Math.pow(h,2)-g-g+l,l=Math.round(n,2)*2,xk=Math.round(h*2)+l,km=Math.round(h*2)+xk,kn=Math.round(n,2)+l+xk-n,ko=Math.pow(cm,2)/10-km,kq=Math.round(km*2),kr=Math.round(l*2)+kn,
/* Mathematical computation on Curtain/blinds animation */
o=function()
{    
 on(o,cm),cm++;if(cm>=cm){on(pa,cm),cm=g,ab();};
 };
